...
class SlackService {
    ...
    async afterStep(test, context, {error, result, duration, passed, retries}) {
        ...
        if (retries.attempts >= 0 && !passed) {
            ...
            if (retries.attempts === retries.limit || retries.limit === 0) {
                ...
                if (error.matcherResult) {
                    ...
                    errorMessage = 'Test failed: something went wrong';
                }
                let testError = errorMessage.replace(/[\u001b\u009b][-[+()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, "");
                ++this.failedSteps;
                ++this.steps;
                const attach = failedAttachment({
                    title: this.testTitle,
                    _currentRetry: retries.attempts
                }, testError, {duration});
        ...
        if (passed) {
            ++this.passedSteps;
            this.attachment.push(passedAttachment({
                title: this.testTitle,
                _currentRetry: retries.attempts
            }, {duration}));
        }
    ...
    async after() {
        if (this.config.framework === `cucumber`) {
            ...
            this.attachment.push({
                author_name: `Total tests: ${this.tests} | Total passed: ${this.passedTests} | Total failed: ${this.failedTests}`,
                color: `#4366c7`
            });
        }
